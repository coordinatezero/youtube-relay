...
Maintains constant YouTube connection, switches between WC input and black frames.
"""
...
YOUR_KEY = "berkshire"
# Local RTMP server for WC input
INPUT_URL = f"rtmp://127.0.0.1:1935/live/{YOUR_KEY}"
...
class WCStreamMonitor:
    """Monitor incoming WC RTMP stream"""
...
class StreamRelay:
    ...
    def __init__(self):
        ...
        self.wc_monitor = WCStreamMonitor(INPUT_URL)
        self.wc_container = None
    ...
    def check_wc_status(self):
        """Check if WC stream is available"""
        return self.wc_monitor.check_stream()
        
    def stream_from_wc(self):
        """Stream frames from WC to YouTube"""
        try:
            # Decode frames from WC and re-encode to YouTube
            for packet in self.wc_monitor.container.demux():
                ...
                for frame in frames:
                    if isinstance(frame, av.VideoFrame):
                        ...
                        if not result:
                            logging.error("stream_from_wc: Failed to send video frame")
                            return False
                        ...
                        for resampled_frame in resampled_frames:
                            if not self.youtube.send_audio_frame(resampled_frame):
                                logging.error("stream_from_wc: Failed to send audio frame")
                                return False

            return False
            
        except Exception as e:
            logging.error(f"Error streaming from WC: {e}")
            return False
#    def stream_from_wc(self):
#        """Stream frames from WC to YouTube"""
#        try:
...
#            return False
    ...
    def stream_black_frames(self):
        ...
        while self.should_run and self.current_mode == "black":
            try:
                ...
                # Check for WC every 3 seconds
                if frame_num % (FPS * 3) == 0:
                    if self.check_wc_status():
    ...
    def run(self):
        ...
        logging.info("="*60)
        logging.info(f"Configure external to stream to: rtmp://{YOUR_SERVER}:1935/live/{YOUR_KEY}")
        logging.info("="*60)
        ...
        while self.should_run:
            ...
            try:
                ...
                if has_external:
                    ...
                    if not self.wc_monitor.open_stream():
                    ...
                    # Stream from WC
                    success = self.stream_from_wc()
                    
                    # WC disconnected
                    self.wc_monitor.close()
                    ...
                    # No WC stream, send black frames
                    self.current_mode = "black"
